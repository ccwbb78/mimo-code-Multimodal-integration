...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("LLM.run")(function* (input: StreamRequest) {
      ...
      // ── Multimodal Bridge: resolve vision model ─────────────────────
      // The bridge itself runs inside ProviderTransform.message() BEFORE
      // unsupportedParts() strips images. Here we just resolve the vision
      // model so it can be passed to the middleware.
      let visionLanguage: LanguageModelV3 | undefined
      if (
        cfg.experimental?.multimodal_bridge &&
        !input.small &&
        ProviderTransform.needsMultimodalBridge(input.model)
      ) {
        // Try user-configured model first, then fall back through known vision model IDs
        const configuredID = cfg.experimental.multimodal_bridge_vision_model
        const candidateIDs = configuredID
          ? [configuredID]
          : ["mimo-v2.5", "mimo-auto", "mimo-v2.5-vision"]
        const providerIDs = [input.model.providerID, input.model.providerID === "mimo" ? "xiaomi" : "mimo"]

        for (const pid of providerIDs) {
          if (visionLanguage) break
          for (const mid of candidateIDs) {
            try {
              const visionModel = yield* provider.getModel(pid as any, mid as any)
              visionLanguage = yield* provider.getLanguage(visionModel)
              l.info("multimodal bridge: vision model resolved", {
        ...
        if (!visionLanguage) {
          l.error("multimodal bridge: no vision model found", {
            tried: candidateIDs.flatMap((mid) => providerIDs.map((pid) => `${pid}/${mid}`)),
            hint: "Set experimental.multimodal_bridge_vision_model in mimocode.json to a valid vision model ID",
          })
        }
      }
