...
export function Prompt(props: PromptProps) {
  ...
  const multimodalBridgeEnabled = createMemo(() => sync.data.config.experimental?.multimodal_bridge ?? false)
  ...
  command.register(() => {
    return [
      ...
      {
        title: multimodalBridgeEnabled() ? t("tui.command.multimodal_bridge.title_on") : t("tui.command.multimodal_bridge.title_off"),
        value: "multimodal_bridge",
        category: "prompt",
        slash: {
          name: "multimodal",
        },
        onSelect: async () => {
          const next = !multimodalBridgeEnabled()
          await sdk.client.global.config.update({
            config: {
              experimental: {
                multimodal_bridge: next,
              },
            },
          })
          await sync.bootstrap()
          toast.show({
            message: next ? t("tui.multimodal_bridge.enabled") : t("tui.multimodal_bridge.disabled"),
            variant: "info",
            duration: 3000,
          })
        },
