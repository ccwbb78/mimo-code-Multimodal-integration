...
export async function message(
  ...
  // Run multimodal bridge BEFORE unsupportedParts() — the bridge converts
  // images to text descriptions so unsupportedParts() won't strip them.
  if (visionModel && needsMultimodalBridge(model) && hasImageContent(msgs)) {
    try {
      msgs = await multimodalBridge(msgs, visionModel)
    } catch (err) {
      // Non-fatal: fall through to unsupportedParts() which will show the
      // "model does not support image input" error for each image.
      console.error("[multimodal bridge] vision analysis failed:", err)
    }
  }
...
/**
 * Multimodal bridge: sends images to a vision model (mimo-v2.5) and replaces
 * image parts with text descriptions so that a text-only reasoning model
 * (mimo-v2.5-pro) can reason about visual content.
 *
 ...
 *
 * The bridge sends all images to the vision model in a single call, then
 * replaces each image part with a text part prefixed by `[多模态视觉分析]`.
 */
export async function multimodalBridge(
  msgs: ModelMessage[],
  visionModel: LanguageModelV3,
): Promise<ModelMessage[]> {
  // Collect all image parts from user messages — covers both `image` type
  // parts and `file` type parts with image mediaTypes (clipboard paste).
  ...
  for (const msg of msgs) {
    ...
    for (const part of msg.content) {
      if (part.type === "image") {
        const imageStr = String(part.image)
        const mediaType = imageStr.startsWith("data:")
          ? imageStr.split(";")[0].replace("data:", "")
          : "image/png"
        imageParts.push({ data: imageStr, mediaType })
      } else if (part.type === "file" && part.mediaType?.startsWith("image/")) {
        // File-type image parts (e.g. clipboard paste) — normalize to data URL
        const raw = typeof part.data === "string" ? part.data : String(part.data)
        const mediaType = part.mediaType
        // If already a data URL, use as-is; otherwise wrap raw base64
        const data = raw.startsWith("data:") ? raw : `data:${mediaType};base64,${raw}`
    ...
    messages: [
      {
        ...
        content: [
          ...
          {
            type: "text" as const,
            text: "请详细描述以上所有图片的内容，保留对编程和代码相关的重要细节。包括：代码片段、错误信息、UI布局、文件结构、终端输出、数据可视化等任何与软件开发相关的信息。如果有多张图片，请分别描述每张图片的内容。",
          },
        ],
      },
    ],
  ...
  // Replace image parts (both `image` and `file` types) with text description
  return msgs.map((msg) => {
    if (msg.role !== "user" || !Array.isArray(msg.content)) return msg
    ...
    const newContent = msg.content.map((part) => {
      if (part.type === "image") {
        return { type: "text" as const, text: "[多模态视觉分析] " + description }
      }
...
/**
 * Check if the multimodal bridge should activate for a given model.
 * Returns true when the model is mimo/xiaomi v2.5-pro (text-only reasoning
 * model that needs images routed through the vision model).
 */
export function needsMultimodalBridge(model: Provider.Model): boolean {
  if (model.providerID !== "mimo" && model.providerID !== "xiaomi") return false
  const id = model.id.toLowerCase()
  return id.includes("v2.5-pro")
}

/**
 * Check if messages contain any image parts that would trigger the bridge.
 * Covers both `image` type parts and `file` type parts with image media types
 * (e.g. clipboard paste creates `file` parts with mediaType "image/png").
 */
export function hasImageContent(msgs: ModelMessage[]): boolean {
  return msgs.some(
    (msg) =>
      msg.role === "user" &&
